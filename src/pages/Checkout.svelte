<script>
    ...
    onMount(() => {
        ...
        if($cartTotal > 0) {
            stripe = Stripe('pk_test_51IAWdVCr7JaRUmLwjr77moy5SAFzPzZQadqX0hTfO8ZdXnGbMyQLoEl2IjJintErLZXRYvDUWp7gh2vsadmMWlsa00oAYbyBMi');
...
{#if $cartTotal > 0}
    <section class="form">
        ...
        <form class="checkout-form" on:submit|preventDefault={handleSubmit}>
            ...
            {/if}
            <!-- submit button-->
            <button type="submit" class="btn btn-block btn-primary" disable={isEmpty} class:disabled={isEmpty}>submit</button>
        </form>
    </section>
{:else}
    <div class="checkout-empty">
        <h2>your cart is empty</h2>
        <a href="/produts" use:link class="btn btn-primary">fill it</a>
    </div>
{/if}